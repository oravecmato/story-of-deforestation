...
// `[referenceYear, referenceYear + horizonYears(horizon)]` — with no new endpoint and no refetch
// beyond the horizon control (and the baseline, which sets the forgone-sink depth) that drive the
// charts above. Kept Vue/Pinia-free so it is unit-testable; the `EquivalenceStrip` component is the
// only Pinia-aware seam (reads the DTO + horizon + locale and calls these).
...
/**
 * Reduce the global DTO into the strip's four Mt CO₂ magnitudes over the forward window
